...
public class LogConfig {
	 ...
	 * ### log文件的格式
	 * 
	 * ### 输出格式解释： ### [%-d{yyyy-MM-dd HH:mm:ss}][Class: %c.%M(%F:%L)] %n[Level:
	 * %-5p] - Msg: %m%n
	 * 
	 * ### %d{yyyy-MM-dd HH:mm:ss}: 时间，大括号内是时间格式 ### %c: 全类名 ### %M: 调用的方法名称 ###
	 * %F:%L 类名:行号（在控制台可以追踪代码） ### %n: 换行 ### %p:
	 * 日志级别，这里%-5p是指定的5个字符的日志名称，为的是格式整齐 ### %m: 日志信息
	 * 
	 * ### 输出的信息大概如下： ### [时间{时间格式}][信息所在的class.method(className：lineNumber)] 换行
	 ...
	 * ### LogCat控制台输出格式
	 * 
	 * ### [Class: 信息所在的class.method(className：lineNumber)] 换行 ### [Level:
