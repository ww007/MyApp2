...
public class ArcProgressBar extends View {
	 ...
	 * 线条高度
	 */
	private int mDottedLineHeight = 2;// '''''''
	/**
	 * 圆弧跟虚线之间的距离
	 */
	 ...
	 * 进度文字大小
	 */
	private int mProgressTextSize = 25;
	/**
	 * 进度描述
	 */
	...
	public void setmArcText(String string) {
		mArcText = string;
	}
	...
	private void initView() {
		...
		mTextPaint.setTextSize(dp2px(getResources(), 14));
		...
		mRonudRectPaint.setColor(mArcBgColor);
...
//		double sqrt = Math.sqrt(mArcRadius * mArcRadius + mArcRadius * mArcRadius);
		...
		mRountRect.left = (float) (2 * mArcCenterX - 2 * aDistance) / 2 - mArcWidth / 2 + 25;////////////
		mRountRect.top = (float) (mArcCenterX + bDistance) - 20;
		mRountRect.right = (float) (2 * mArcCenterX - (2 * mArcCenterX - 2 * aDistance) / 2) - mArcWidth / 2 - 20;
		mRountRect.bottom = (float) (mArcRadius + mArcRadius) - 10;
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawText(mArcText, mArcRadius - mTextPaint.measureText(mArcText) / 2,
				(float) (mArcRadius + bDistance) - 2 * (mTextPaint.descent() + mTextPaint.ascent()+5), mTextPaint);
		// Log.i(TAG, (float) (mArcRadius + bDistance) - 2 *
	...
	private void drawRunText(Canvas canvas) {
		...
		canvas.drawText(progressStr, mArcCenterX - mProgressPaint.measureText(progressStr) / 2,
				mArcCenterX - (mProgressPaint.descent() + mProgressPaint.ascent()) / 2 - 10, mProgressPaint);
	}
