...
public class JumpHeightActivity extends NFCActivity {
	...
	private Logger log = Logger.getLogger(JumpHeightActivity.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		List<Item> items = DbService.getInstance(context).queryItemByName("摸高");
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == 66 || keyCode == 135 || keyCode == 136) {
			Intent intent = new Intent(JumpHeightActivity.this, CaptureActivity.class);
			intent.putExtra("className", Constant.JUMP_HEIGHT + "");
	...
	private void setListener() {
		...
		rg.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				RadioButton radioButton = (RadioButton) JumpHeightActivity.this
		...
		btnSave.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if ("".equals(etChengji.getText().toString()) && checkedBtn.equals("正常")) {
					Toast.makeText(JumpHeightActivity.this, "成绩为空", Toast.LENGTH_SHORT).show();
					return;
				} else if (checkedBtn.equals("犯规")) {
					resultState = -1;
				} else if (checkedBtn.equals("弃权")) {
					...
					// 查询数据库中保存的该学生项目成绩的轮次
					String itemCode = DbService.getInstance(context).queryItemByMachineCode(Constant.JUMP_HEIGHT + "")
							.get(0).getItemCode();
					// long stuId =
