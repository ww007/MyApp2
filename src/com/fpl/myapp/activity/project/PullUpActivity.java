...
public class PullUpActivity extends NFCActivity {
	...
	private Logger log = Logger.getLogger(PullUpActivity.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		List<Item> items = DbService.getInstance(context).queryItemByName("引体向上");
	...
	private void readCard(Intent intent) {
		try {
			...
			log.info("引体向上读卡=>" + student.toString());
			if (1 == student.getSex()) {
				sex = "男";
			} else {
				sex = "女";
	...
	protected void initView() {
		...
		tvInfoTitle.setText("引体向上");
		tvInfoChengji.setText("引体向上");
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == 66 || keyCode == 135 || keyCode == 136) {
			Intent intent = new Intent(PullUpActivity.this, CaptureActivity.class);
			intent.putExtra("className", Constant.PULL_UP + "");
	...
	private void setListener() {
		...
		rg.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				RadioButton radioButton = (RadioButton) PullUpActivity.this
		...
		btnSave.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if ("".equals(etChengji.getText().toString()) && checkedBtn.equals("正常")) {
					Toast.makeText(PullUpActivity.this, "成绩为空", Toast.LENGTH_SHORT).show();
					return;
				} else if (checkedBtn.equals("犯规")) {
					resultState = -1;
				} else if (checkedBtn.equals("弃权")) {
					...
					// 查询数据库中保存的该学生项目成绩的轮次
					String itemCode = DbService.getInstance(context).queryItemByMachineCode(Constant.PULL_UP + "")
							.get(0).getItemCode();
					// long stuId =
