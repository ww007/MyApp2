 ...
 *
 */
public class ShuttlecockKickingActivity extends NFCActivity {
	...
	private void readCard(Intent intent) {
		try {
			...
			log.info("踢毽子读卡=>" + student.toString());
			if (1 == student.getSex()) {
				sex = "男";
			} else {
				sex = "女";
	...
	protected void initView() {
		...
		tvInfoTitle.setText("踢毽子");
		tvInfoChengji.setText("踢毽子");
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == 66 || keyCode == 135 || keyCode == 136) {
			Intent intent = new Intent(ShuttlecockKickingActivity.this, CaptureActivity.class);
			intent.putExtra("className", Constant.KICKING_SHUTTLECOCK + "");
	...
	private void setListener() {
		...
		rg.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				RadioButton radioButton = (RadioButton) ShuttlecockKickingActivity.this
		...
		btnSave.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				int flag = SaveDBUtil.saveGradesDB(context, tvNumber.getText().toString(), grade, resultState,
						Constant.KICKING_SHUTTLECOCK + "", "踢毽子");
