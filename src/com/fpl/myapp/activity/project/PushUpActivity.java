...
public class PushUpActivity extends NFCActivity {
	...
	private Logger log = Logger.getLogger(PushUpActivity.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		List<Item> items = DbService.getInstance(context).queryItemByName("俯卧撑");
	...
	private void readCard(Intent intent) {
		try {
			...
			log.info("俯卧撑读卡=>" + student.toString());
			if (1 == student.getSex()) {
				sex = "男";
			} else {
				sex = "女";
	...
	protected void initView() {
		...
		tvInfoTitle.setText("俯卧撑");
		tvInfoChengji.setText("俯卧撑");
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == 66 || keyCode == 135 || keyCode == 136) {
			Intent intent = new Intent(PushUpActivity.this, CaptureActivity.class);
			intent.putExtra("className", Constant.PUSH_UP + "");
	...
	private void setListener() {
		...
		rg.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				RadioButton radioButton = (RadioButton) PushUpActivity.this
		...
		btnSave.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if ("".equals(etChengji.getText().toString()) && checkedBtn.equals("正常")) {
					Toast.makeText(PushUpActivity.this, "成绩为空", Toast.LENGTH_SHORT).show();
					return;
				} else if (checkedBtn.equals("犯规")) {
					resultState = -1;
				} else if (checkedBtn.equals("免测")) {
					resultState = -2;
					...
					// 查询数据库中保存的该学生项目成绩的轮次
					String itemCode = DbService.getInstance(context).queryItemByMachineCode(Constant.PUSH_UP + "")
							.get(0).getItemCode();
					// long stuId =
