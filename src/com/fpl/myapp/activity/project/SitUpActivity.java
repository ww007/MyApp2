...
public class SitUpActivity extends NFCActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		List<Item> items = DbService.getInstance(context).queryItemByMachineCode("6");
		...
		btnSave.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if ("".equals(etChengji.getText().toString()) && checkedBtn.equals("正常")) {
					...
					// 查询数据库中保存的该学生项目成绩的轮次
					String itemCode = DbService.getInstance(context).queryItemByMachineCode(Constant.SIT_UP + "").get(0)
							.getItemCode();
					// long stuId =
