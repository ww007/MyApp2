...
public class InfraredBallActivity extends NFCActivity {
	...
	private void readCard(Intent intent) {
		...
		try {
			...
			log.info("红外实心球读卡->" + student.toString());
	...
	protected void initView() {
		...
		tvInfoTitle.setText("红外实心球");
		tvInfoChengji.setText("红外实心球");
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == 66 || keyCode == 135 || keyCode == 136) {
			Intent intent = new Intent(InfraredBallActivity.this, CaptureActivity.class);
			intent.putExtra("className", Constant.INFRARED_BALL + "");
	...
	private void setListener() {
		...
		rg.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				RadioButton radioButton = (RadioButton) InfraredBallActivity.this
		...
		btnSave.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if ("".equals(etChengji.getText().toString()) && checkedBtn.equals("正常")) {
					Toast.makeText(InfraredBallActivity.this, "成绩为空", Toast.LENGTH_SHORT).show();
					return;
				} else if (checkedBtn.equals("犯规")) {
					resultState = -1;
				} else if (checkedBtn.equals("弃权")) {
